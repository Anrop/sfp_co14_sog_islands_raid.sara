...
ENGTASKS_CreateTaskLocal = {
	...
	if (count _taskTargetDestination > 0) then {
		...
		if (_isDestination) then {
			_sFnc = _taskName + " setSimpleTaskDestination " + str _taskTargetDestination + ";";
			call compile _sFnc;
		};
		if (_isTarget) then {
			_sFnc = _taskName + " setSimpleTaskTarget " + str _taskTargetDestination + ";";
			call compile _sFnc;
		};
	};
...
ENGTASKS_SetTaskDestinationLocal = {
	...
	_taskDescription = (call compile _sFnc) select 0;
